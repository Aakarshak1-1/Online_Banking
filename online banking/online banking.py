...
def import_and_create_bank(filename):
    ...
    for i in _lines:
        ...
        _value = i[1].strip()
        try:
            bank[_key] = bank.get(_key, 0)+float(_value)
        except:
            continue
    _file.close()
    return bank
...
tools.assert_false(len(
    bank) == 0, "The bank dictionary is not supposed to be empty after reading 'bank.txt'.")
tools.assert_almost_equal(115.5, bank.get(
    "Brandon"), msg="The total amount for Brandon is incorrect. It should be 115.5.")
tools.assert_almost_equal(128.87, bank.get(
    "James"), msg="The total amount for James is incorrect. It should be 128.87.")
tools.assert_is_none(
    bank.get("Joel"), "Joel should not be in the bank dictionary.")
tools.assert_is_none(
    bank.get("Luke"), "Luke should not be in the bank dictionary.")
tools.assert_almost_equal(827.43, bank.get(
    "Sarah"), msg="The total amount for Sarah is incorrect. It should be 827.43.")
print("Success!")
...
def signup(user_accounts, log_in, username, password):
    ...
    # your code here:
    # flow:
    # func_to define valid password
    # check username in user_accounts True
    # else check password True update both dictionary  returns true
    # else false
    def valid_password(username, password):
        prev, curr = 0, 0

        if (any(x.isupper() for x in password) and any(x.islower() for x in password) and any(x.isdigit() for x in password) and len(password) >= 8 and (username != password)):
            return True
...
tools.assert_false(len(
    log_in) == 0, "The login dictionary is not supposed to be empty after reading 'user.txt'.")
tools.assert_equal("brandon123ABC", user_accounts.get(
    "Brandon"), "The password associated with username 'Brandon' is incorrect.")
tools.assert_equal("jack123POU", user_accounts.get(
    "Jack"), "The password associated with username 'Jack' is incorrect.")
tools.assert_is_none(user_accounts.get(
    "Jennie"), "Jennie should not appear in user_accounts, since the associated password was invalid.")
tools.assert_false(
...
tools.assert_true("BrandonK" in user_accounts,
                  "BrandonK is not in user_accounts.")
tools.assert_equal("123aABCD", user_accounts["BrandonK"],
                   "The password associated with'BrandonK' is incorrect.")
tools.assert_false(
    log_in["BrandonK"], "'BrandonK' has not logged in yet, so this should be false initially.")
print("Success!")
...
def login(user_accounts, log_in, username, password):
    ...
    # your code here
    if(username not in user_accounts.keys() or password not in user_accounts.values()):
        return False
    else:
...
tools.assert_false(login(user_accounts, log_in, "BrandonK", "123aABCD"),
...
def update(bank, log_in, username, amount):
    ...
    # your code here
    if(log_in[username] == True and amount > 0):
...
tools.assert_false(update(bank, log_in, "Jack", 100),
                   "When the user is not logged in, return False.")
...
tools.assert_almost_equal(bank.get(
    "Brandon"), 215.5, msg="After the update, the total amount for Brandon is incorrect. It should be 215.5.")

signup(user_accounts, log_in, "BrandonK", "123aABCD")
tools.assert_is_none(bank.get("BrandonK"),
                     "'BrandonK' should not be in the bank dictionary yet.")
...
tools.assert_almost_equal(100, bank.get(
    "BrandonK"), msg="After the update, the total amount for 'BrandonK' is incorrect. It should be 100.")
print("Success!")
...
def transfer(bank, log_in, userA, userB, amount):
    ...
    '''
    # your code here
    if(userA in bank.keys() and log_in[userA] == True and userB in log_in.keys() and bank[userA] >= amount and amount > 0):
        try:
            bank[userA] = bank.get(userA)-amount
            bank[userB] = bank.get(userB)+amount
            return True
        except:
            bank[userB] = bank.get(userB, 0)
            bank[userB] = bank.get(userB)+amount
            return True
# In[ ]:
...
tools.assert_almost_equal(105.5, bank.get(
    "Brandon"), msg="After the transfer, the total amount for 'Brandon' is incorrect. It should be 105.5.")
tools.assert_almost_equal(55, bank.get(
    "Jack"), msg="After the transfer, the total amount for 'Jack' is incorrect. It should be 55.")
...
tools.assert_almost_equal(bank.get(
    "Brandon"), 95.5, msg="After the transfer, thee total amount for 'Brandon' is incorrect. It should be 95.5.")
tools.assert_almost_equal(bank.get(
    "BrandonK"), 10, msg="After the transfer, the total amount for 'BrandonK' is incorrect. It should be 10.")
print("Success!")
...
def change_password(user_accounts, log_in, username, old_password, new_password):
    ...
    # your code here
    def valid_password(username, password):
        if(username != password and len(password) >= 8 and (any(x.isdigit()) for x in password) and (any(x.islower()) for x in password) and (any(x.isupper()) for x in password)):
            return True
        else:
            return False
    # curr = ""
    if(username in user_accounts and log_in[username] == True):
        check = valid_password(username, new_password)
        if(check == True and old_password != new_password and user_accounts[username] == old_password):
...
tools.assert_false(change_password(user_accounts, log_in, "BrandonK", "123abcABC",
                   "123abcABCD"), "BrandonK is not in user_accounts yet, so this should return False.")
tools.assert_false(change_password(user_accounts, log_in, "Brandon", "brandon123ABC",
                   "123abcABCD"), "A user must be logged in to change the password.")

login(user_accounts, log_in, "Brandon", "brandon123ABC")
tools.assert_false(change_password(user_accounts, log_in, "Brandon", "123abcABCD",
                   "123abcABCDE"), "The old password entered should be the same as the current password.")

tools.assert_false(change_password(user_accounts, log_in, "Brandon", "brandon123ABC",
                   "brandon123ABC"), "The new password should be different from the old one.")

tools.assert_false(change_password(user_accounts, log_in, "Brandon",
                   "brandon123ABC", "123ABCD"), "The new password should be valid.")

tools.assert_true(change_password(user_accounts, log_in, "Brandon", "brandon123ABC",
                  "123abcABCD"), "This function should return True when there are no issues.")
...
def delete_account(user_accounts, log_in, bank, username, password):
    ...
    # your code here
    if(username in user_accounts.keys() and user_accounts[username] == password and log_in[username] == True):
...
tools.assert_false(delete_account(user_accounts, log_in, bank, "Brandon",
                   "brandon123ABC"), "'Brandon' is not logged in yet, so this should return False.")

login(user_accounts, log_in, "Brandon", "brandon123ABC")
tools.assert_false(delete_account(user_accounts, log_in, bank, "Brandon",
                   "123abcABDC"), "The password is incorrect, so this should return False.")
...
tools.assert_is_none(user_accounts.get(
    "Brandon"), "After deleting the account, 'Brandon' should not be in user_accounts.")
tools.assert_is_none(log_in.get(
    "Brandon"), "After deleting the account, 'Brandon' should not be in log_in.")
tools.assert_is_none(bank.get(
    "Brandon"), "After deleting the account, 'Brandon' should not be in bank.")
print("Success!")
...
def main():
    ...
    while True:
        ...
        if option == "1":
            ...
            change_password(user_accounts, log_in, username,
